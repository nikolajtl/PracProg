...
int main() {
    ...
    // Write vectors (wavefunctions) to file
    std::ofstream funcfile("wavefuncs.txt");
    if (!funcfile) {
        std::cerr << "Error opening file!" << std::endl;
        return 1;
    };

    int plotstates = 3; // Number of wavefunctions to plot
    // Write headers
    funcfile << "r ";
    for (int j = 0; j < plotstates; j++) {
        funcfile << "𝛙_" << j + 1 << " ";
    };
    funcfile << std::endl;

    std::vector<std::vector<double>> V = transposeMatrix(solution.V); // Åbenbart omvendt sorteret eller noget
    for (int i=0; i < rs.size(); i++) {
        funcfile << rs[i] << " ";
        for (int j=0; j<plotstates; j++) {
            funcfile << V[j][i] / std::sqrt(delta_r) << " "; // See exercise for why the constant is as such
        };
        funcfile << std::endl;
    };

    funcfile.close();
    std::cout << "Wavefunctions (the first few) written to wavefuncs.txt" << std::endl;
